interface ExerciseInfo {
  exercise: number
  tier: number
  title: string
  course_id: string
}

export const BAIexercises: Record<string, ExerciseInfo> = {
  "95054": {
    exercise: 1,
    tier: 3,
    title: "Exercise 1: Listing pineapple routes",
    course_id: "0e9d1a22-0e19-4320-8c8c-84115bb26452",
  },
  "95055": {
    exercise: 1,
    tier: 2,
    title: "Exercise 1: Listing pineapple routes",
    course_id: "3a2790fc-227c-4045-9f4c-40a2bdabe76a",
  },
  "95056": {
    exercise: 2,
    tier: 3,
    title: "Exercise 2: Pineapple route emissions",
    course_id: "0e9d1a22-0e19-4320-8c8c-84115bb26452",
  },
  "95057": {
    exercise: 2,
    tier: 2,
    title: "Exercise 2: Pineapple route emissions",
    course_id: "3a2790fc-227c-4045-9f4c-40a2bdabe76a",
  },
  "95058": {
    exercise: 3,
    tier: 3,
    title: "Exercise 3: Reach the highest summit",
    course_id: "0e9d1a22-0e19-4320-8c8c-84115bb26452",
  },
  "95059": {
    exercise: 3,
    tier: 2,
    title: "Exercise 3: Reach the highest summit",
    course_id: "3a2790fc-227c-4045-9f4c-40a2bdabe76a",
  },
  "95060": {
    exercise: 4,
    tier: 3,
    title: "Exercise 4: Probabilities",
    course_id: "0e9d1a22-0e19-4320-8c8c-84115bb26452",
  },
  "95061": {
    exercise: 4,
    tier: 2,
    title: "Exercise 4: Probabilities",
    course_id: "3a2790fc-227c-4045-9f4c-40a2bdabe76a",
  },
  "95062": {
    exercise: 5,
    tier: 3,
    title: "Exercise 5: Warm-up Temperature",
    course_id: "0e9d1a22-0e19-4320-8c8c-84115bb26452",
  },
  "95063": {
    exercise: 6,
    tier: 3,
    title: "Exercise 6: Simulated Annealing",
    course_id: "0e9d1a22-0e19-4320-8c8c-84115bb26452",
  },
  "95064": {
    exercise: 6,
    tier: 2,
    title: "Exercise 6: Simulated Annealing",
    course_id: "3a2790fc-227c-4045-9f4c-40a2bdabe76a",
  },
  "98214": {
    exercise: 7,
    tier: 3,
    title: "Exercise 7: Flip the coin",
    course_id: "0e9d1a22-0e19-4320-8c8c-84115bb26452",
  },
  "98215": {
    exercise: 7,
    tier: 2,
    title: "Exercise 7: Flip the coin",
    course_id: "3a2790fc-227c-4045-9f4c-40a2bdabe76a",
  },
  "98216": {
    exercise: 8,
    tier: 3,
    title: "Exercise 8: Fishing in the Nordics",
    course_id: "0e9d1a22-0e19-4320-8c8c-84115bb26452",
  },
  "98217": {
    exercise: 8,
    tier: 2,
    title: "Exercise 8: Fishing in the Nordics",
    course_id: "3a2790fc-227c-4045-9f4c-40a2bdabe76a",
  },
  "98218": {
    exercise: 9,
    tier: 3,
    title: "Exercise 9: Block or not",
    course_id: "0e9d1a22-0e19-4320-8c8c-84115bb26452",
  },
  "98219": {
    exercise: 10,
    tier: 3,
    title: "Exercise 10: Naive Bayes classifier",
    course_id: "0e9d1a22-0e19-4320-8c8c-84115bb26452",
  },
  "98220": {
    exercise: 10,
    tier: 2,
    title: "Exercise 10: Naive Bayes classifier",
    course_id: "3a2790fc-227c-4045-9f4c-40a2bdabe76a",
  },
  "98893": {
    exercise: 11,
    tier: 3,
    title: "Exercise 11: Real estate price predictions",
    course_id: "0e9d1a22-0e19-4320-8c8c-84115bb26452",
  },
  "98894": {
    exercise: 11,
    tier: 2,
    title: "Exercise 11: Real estate price predictions",
    course_id: "3a2790fc-227c-4045-9f4c-40a2bdabe76a",
  },
  "98895": {
    exercise: 12,
    tier: 3,
    title: "Exercise 12: Least squares",
    course_id: "0e9d1a22-0e19-4320-8c8c-84115bb26452",
  },
  "98896": {
    exercise: 12,
    tier: 2,
    title: "Exercise 12: Least squares",
    course_id: "3a2790fc-227c-4045-9f4c-40a2bdabe76a",
  },
  "98897": {
    exercise: 13,
    tier: 3,
    title: "Exercise 13: Predictions with more data",
    course_id: "0e9d1a22-0e19-4320-8c8c-84115bb26452",
  },
  "98898": {
    exercise: 13,
    tier: 2,
    title: "Exercise 13: Predictions with more data",
    course_id: "3a2790fc-227c-4045-9f4c-40a2bdabe76a",
  },
  "98899": {
    exercise: 14,
    tier: 3,
    title: "Exercise 14: Training data vs test data",
    course_id: "0e9d1a22-0e19-4320-8c8c-84115bb26452",
  },
  "98900": {
    exercise: 14,
    tier: 2,
    title: "Exercise 14: Training data vs test data",
    course_id: "3a2790fc-227c-4045-9f4c-40a2bdabe76a",
  },
  "98901": {
    exercise: 15,
    tier: 3,
    title: "Exercise 15: Vector distances",
    course_id: "0e9d1a22-0e19-4320-8c8c-84115bb26452",
  },
  "98902": {
    exercise: 16,
    tier: 3,
    title: "Exercise 16: Nearest neighbor",
    course_id: "0e9d1a22-0e19-4320-8c8c-84115bb26452",
  },
  "98903": {
    exercise: 16,
    tier: 2,
    title: "Exercise 16: Nearest neighbor",
    course_id: "3a2790fc-227c-4045-9f4c-40a2bdabe76a",
  },
  "98912": {
    exercise: 17,
    tier: 3,
    title: "Exercise 17: Bag of words",
    course_id: "0e9d1a22-0e19-4320-8c8c-84115bb26452",
  },
  "98913": {
    exercise: 17,
    tier: 2,
    title: "Exercise 17: Bag of words",
    course_id: "3a2790fc-227c-4045-9f4c-40a2bdabe76a",
  },
  "98914": {
    exercise: 18,
    tier: 3,
    title: "Exercise 18: TF-IDF",
    course_id: "0e9d1a22-0e19-4320-8c8c-84115bb26452",
  },
  "98915": {
    exercise: 18,
    tier: 2,
    title: "Exercise 18: TF-IDF",
    course_id: "3a2790fc-227c-4045-9f4c-40a2bdabe76a",
  },
  "1e4af21e-7734-5282-a070-cc464a376e81": {
    exercise: 1,
    tier: 1,
    title: "Exercise 1: Listing pineapple routes",
    course_id: "e1eaff32-8b2c-4423-998d-d3477535a1f9",
  },
  "7252cbff-92b4-5766-9f9c-64558a577de2": {
    exercise: 2,
    tier: 1,
    title: "Exercise 2: Pineapple route emissions",
    course_id: "e1eaff32-8b2c-4423-998d-d3477535a1f9",
  },
  "eaf29-d6d8-5837-9db1-2a1dec51a1a1": {
    exercise: 3,
    tier: 1,
    title: "Exercise 3: Reach the highest summit",
    course_id: "e1eaff32-8b2c-4423-998d-d3477535a1f9",
  },
  "97339e0e-6dfc-59ea-8338-b516dee0b893": {
    exercise: 5,
    tier: 1,
    title: "Exercise 5: Warm-up Temperature",
    course_id: "e1eaff32-8b2c-4423-998d-d3477535a1f9",
  },
  "da433f82-b563-5b17-a580-3276ae851e3a": {
    exercise: 5,
    tier: 2,
    title: "Exercise 5: Warm-up Temperature",
    course_id: "3a2790fc-227c-4045-9f4c-40a2bdabe76a",
  },
  "3ae47ac2-d2d7-5d3c-8823-dc6e32728340": {
    exercise: 6,
    tier: 1,
    title: "Exercise 6: Simulated Annealing",
    course_id: "e1eaff32-8b2c-4423-998d-d3477535a1f9",
  },
  "cd59d296-8f49-5ff5-97b3-d9d274a555e0": {
    exercise: 7,
    tier: 1,
    title: "Exercise 7: Flip the coin",
    course_id: "e1eaff32-8b2c-4423-998d-d3477535a1f9",
  },
  "43123f05-dec1-5c84-909a-e20e8ce30c63": {
    exercise: 8,
    tier: 1,
    title: "Exercise 8: Fishing in the Nordics",
    course_id: "e1eaff32-8b2c-4423-998d-d3477535a1f9",
  },
  "2537e870-4b2e-57c9-9b90-83f63e58d8c6": {
    exercise: 9,
    tier: 1,
    title: "Exercise 9: Block or not",
    course_id: "e1eaff32-8b2c-4423-998d-d3477535a1f9",
  },
  "11b5bfa5-0b82-5e80-be0c-e61c91fb7416": {
    exercise: 9,
    tier: 2,
    title: "Exercise 9: Block or not",
    course_id: "3a2790fc-227c-4045-9f4c-40a2bdabe76a",
  },
  "47395118-16ba-5b98-a829-8aa53baf9e6c": {
    exercise: 10,
    tier: 1,
    title: "Exercise 10: Naive Bayes classifier",
    course_id: "e1eaff32-8b2c-4423-998d-d3477535a1f9",
  },
  "a1e05151-e63c-53ad-9da5-020c01fc00bd": {
    exercise: 11,
    tier: 1,
    title: "Exercise 11: Real estate price predictions",
    course_id: "e1eaff32-8b2c-4423-998d-d3477535a1f9",
  },
  "b8c3a491-0b81-57d6-a404-553c1009ec86": {
    exercise: 12,
    tier: 1,
    title: "Exercise 12: Least squares",
    course_id: "e1eaff32-8b2c-4423-998d-d3477535a1f9",
  },
  "db4b9a3f-6a6d-57a6-bcd1-d247685d55e7": {
    exercise: 13,
    tier: 1,
    title: "Exercise 13: Predictions with more data",
    course_id: "e1eaff32-8b2c-4423-998d-d3477535a1f9",
  },
  "f558ab79-f749-5168-add1-d2bf8b141f51": {
    exercise: 14,
    tier: 1,
    title: "Exercise 14: Training data vs test data",
    course_id: "e1eaff32-8b2c-4423-998d-d3477535a1f9",
  },
  "6bc4c453-49c9-5966-9f58-8afc8859f235": {
    exercise: 15,
    tier: 1,
    title: "Exercise 15: Vector distances",
    course_id: "e1eaff32-8b2c-4423-998d-d3477535a1f9",
  },
  "48cee2ae-967f-50f5-9e1e-f2dbe9a8f411": {
    exercise: 15,
    tier: 2,
    title: "Exercise 15: Vector distances",
    course_id: "3a2790fc-227c-4045-9f4c-40a2bdabe76a",
  },
  "c0d27ea9-86b7-596a-b1d2-b5e96b7b7b5e": {
    exercise: 16,
    tier: 1,
    title: "Exercise 16: Nearest neighbor",
    course_id: "e1eaff32-8b2c-4423-998d-d3477535a1f9",
  },
  "a9964889-6f37-5696-bf58-431050566a1a": {
    exercise: 17,
    tier: 1,
    title: "Exercise 17: Bag of words",
    course_id: "e1eaff32-8b2c-4423-998d-d3477535a1f9",
  },
  "d44b7655-af73-5b18-99e7-a2760359c137": {
    exercise: 18,
    tier: 1,
    title: "Exercise 18: TF-IDF",
    course_id: "e1eaff32-8b2c-4423-998d-d3477535a1f9",
  },
  "b8c62bf4-c4b2-544c-a8e9-4d757d6adb77": {
    exercise: 19,
    tier: 1,
    title: "Exercise 19: Looking out for overfitting",
    course_id: "e1eaff32-8b2c-4423-998d-d3477535a1f9",
  },
  "deb6022a-9c1a-5fbf-a238-f69ecbede584": {
    exercise: 19,
    tier: 2,
    title: "Exercise 19: Looking out for overfitting",
    course_id: "3a2790fc-227c-4045-9f4c-40a2bdabe76a",
  },
  "538d4271-4dbf-402f-8e7a-4a9af85d635b": {
    exercise: 19,
    tier: 3,
    title: "Exercise 19: Looking out for overfitting",
    course_id: "0e9d1a22-0e19-4320-8c8c-84115bb26452",
  },
  "cb006881-955d-5205-925c-2a11e1666fd7": {
    exercise: 20,
    tier: 1,
    title: "Exercise 20: Logistic regression",
    course_id: "e1eaff32-8b2c-4423-998d-d3477535a1f9",
  },
  "dfbdd7e5-1fc1-53a6-9ab4-866daac27b4a": {
    exercise: 20,
    tier: 2,
    title: "Exercise 20: Logistic regression",
    course_id: "3a2790fc-227c-4045-9f4c-40a2bdabe76a",
  },
  "9364037f-1e74-50c7-8c92-2189d861ff47": {
    exercise: 20,
    tier: 3,
    title: "Exercise 20: Logistic regression",
    course_id: "0e9d1a22-0e19-4320-8c8c-84115bb26452",
  },
  "48f05b72-10fd-5c83-b5a8-c104c7636376": {
    exercise: 21,
    tier: 1,
    title: "Exercise 21: Neural networks",
    course_id: "e1eaff32-8b2c-4423-998d-d3477535a1f9",
  },
  "8db327e4-0c2c-54ca-aa7e-23a66e6c4b6c": {
    exercise: 21,
    tier: 2,
    title: "Exercise 21: Neural networks",
    course_id: "3a2790fc-227c-4045-9f4c-40a2bdabe76a",
  },
  "058c570e-2adc-5285-9d88-a1d2a0b1c05d": {
    exercise: 21,
    tier: 3,
    title: "Exercise 21: Neural networks",
    course_id: "0e9d1a22-0e19-4320-8c8c-84115bb26452",
  },
}

export const BAIbadge = {
  "836dae15-6755-416e-82e8-508afa5b6854": {
    exercise: 22,
    tier: 1,
    title: "Your AI idea",
    course_id: "e1eaff32-8b2c-4423-998d-d3477535a1f9",
  },
  "a2441f0a-9d66-4282-a24b-4318d242a6f0": {
    exercise: 22,
    tier: 2,
    title: "Your AI idea",
    course_id: "3a2790fc-227c-4045-9f4c-40a2bdabe76a",
  },
  "bcbf760d-a203-46bc-9369-a802e73add15": {
    exercise: 22,
    tier: 3,
    title: "Your AI idea",
    course_id: "0e9d1a22-0e19-4320-8c8c-84115bb26452",
  },
}

export const BAItiers: Record<number, string> = {
  1: "e1eaff32-8b2c-4423-998d-d3477535a1f9",
  2: "3a2790fc-227c-4045-9f4c-40a2bdabe76a",
  3: "0e9d1a22-0e19-4320-8c8c-84115bb26452",
}

export const requiredByTestTier: Record<number, number> = {
  1: 2,
  2: 1,
  3: 1,
}

export const pointsTestNeeded = 2
export const exerciseTestCompletionsNeeded = 2

export const pointsNeeded = 10.5
export const exerciseCompletionsNeeded = 19

export const requiredByTier: Record<number, number> = {
  1: 19,
  2: 14,
  3: 14,
}
