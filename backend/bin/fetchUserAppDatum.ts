...
import { Prisma, Course, UserCourseSettings, User } from "../generated/prisma-client";
...
const fetcUserAppDatum = async () => {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        const isUser: Boolean = await prisma.$exists.user({ upstream_id: p.user_id })
        if (!isUser) {
            try {
                await getUserFromTmc(p.user_id, tmc)
            } catch (error) {
                console.log("error in getting user data from tmc, trying again in 30s...")
        ...
        const isOld: Boolean = await prisma.$exists.userCourseSettings({ user: { upstream_id: p.user_id }, course: { id: course.id } })
        if (!isOld) {
    ...
    console.log(config)
    await fs.writeFile("./bin/userAppDatumConfig.json", await JSON.stringify(config), (e) => console.log(e))
