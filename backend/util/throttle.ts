...
export async function throttle(user: any, { knex }: ApiContext) {
    ...
    let passwordThrottle = user.password_throttle || <any>[{ currentRate: 0, limitStamp: null }]
    ...
    passwordThrottle.forEach((t: any) => {
        ...
        throttleData = {
            status: 403,
            success: false,
            message: `Incorrect password. You have ${RATE_LIMIT - t.currentRate} attempts left.`,
        }
        return
    })
