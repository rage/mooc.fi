...
export function authorize({ knex }: ApiContext) {
  return async (req: any, res: any) => {
    ...
    let accessToken = (await knex.select("*").from("prisma2.access_tokens").where("client_id", authorizationCode.client_id).where("user_id", auth.id).where("access_token", auth.nonce).where("valid", true))?.[0]
    if(accessToken) {
